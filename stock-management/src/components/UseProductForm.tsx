...
import { z } from "zod";
import { TextField, Button, MenuItem, Tabs, Tab } from "@mui/material";
    ...
    try {
      ...
      if (product) {
        const updatedProduct = {
          ...product,
          quantity:
            tabValue === 0 ? product.quantity - data.amount : data.amount,
        };
        await dispatch(
          updateProductAsync({ id: data.productId, product: updatedProduct })
        ).unwrap();
        reset();
        onSuccess?.();
      }
          ...
          >
            <Controller
              ...
              render={({
                ...
                <TextField
                  label={
                    tabValue === 0 ? "Azaltılacak Miktar" : "Yeni Stok Miktarı"
                  }
                  type="number"
                  fullWidth
                  value={value}
